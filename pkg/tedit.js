...
pkgs.tedit = function(args,trmnl){
	...
	trmnl.running.tedit.screen.onkeydown = function(e){
		...
		if(e.ctrlKey || e.metaKey){
			...
			if(code == 70){ // 'f'
				e.preventDefault();
				trmnl.running.tedit.consoleInput.value = "find ";
	...
	trmnl.running.tedit.consoleInput.onkeydown = function(e){
		this.setAttribute("placeholder",""); // remove the error-spawned placeholder text on interaction
		var code = e.keyCode || e.which;
		if(code == 13){
			e.preventDefault();
			// this is search. We will parse input:
			var cmd = this.value;
	...
	// set the trmnl.running.tedit.screen.value here, based on if argument was supplied.
...
var TEdit = function(trmnl){
	...
	this.main.appendChild(this.downloadLink);
}
...
TEdit.prototype.find = function(srch){
	var data = this.screen.value.toLowerCase();
	...
	if(ind > -1){
		...
		if(ind > -1){
			...
			this.statusbar.innerHTML = "<span>Search wrapped</span>";
...
TEdit.prototype.statusFade = function(){
	...
	*/
	let that = this;
	setTimeout(function(){that.statusbar.innerHTML = "";},3500);
};
/* Not being used until I code in the above animation again:
...
TEdit.prototype.parse_command = function(cmd){
    ...
    if(resCode){
		...
		this.consoleInput.setAttribute("placeholder",response[1]);
        this.consoleInput.value = "";
    }else{
        this.hideConsole();
    }
...
TEdit.prototype.fn.help = function(args,that){
	...
	that.statusbar.innerHTML = "<span>Help was called. Yet to code. See console for argument/flag details</span>";
	that.statusbar.style.display = "block";
	that.statusFade();
...
TEdit.prototype.fn.replace = function(args,that){
	[args,flags] = terminal[that.termID].parse_flags(args);
	that.statusbar.innerHTML = "<span>Replace was called. Yet to code...</span>";
	that.statusbar.style.display = "block";
	that.statusFade();
	return 0;
};
TEdit.prototype.fn.save = function(args,that){
	[args,flags] = terminal[that.termID].parse_flags(args);
	that.statusbar.innerHTML = "<span>Writing to local file...</span>";
	that.statusbar.style.display = "block";
