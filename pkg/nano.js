...
pkgs.nano = function(args,trmnl){
	...
	$(trmnl.running.nano.screen).keydown(function(e){
		...
		if(e.ctrlKey){
			if(code == 88){ // '^X'
				...
				$(trmnl.running.nano.statusbar).html("<table style='width: 100%;'><tr><td style='width: 1px; white-space: nowrap;'>Search:</td><td><input id='nanosrch' style='width: 100%; background: transparent; border: none; outline: none;'/></td></tr></table>");
				$("#nanosrch").focus();
				$("#nanosrch").keydown(function(e){
					var code = e.keyCode || e.which;
					if(code == 13){
						e.preventDefault();
						trmnl.running.nano.find($(this).val());
					}
				});
			}
	...
	// set the $(trmnl.running.nano.screen).val() here, based on if argument was supplied.
...
var Nano = function(trmnl){
	...
	$(this.titlebar).html("<table style='width: 100%;'><tr><td style='width: 33%;'>emerix nano "+this.version+"</td><td style='width: 33%; text-align: center;'>New buffer</td><td style='width: 33%;'>&nbsp;</td>"); // adjust this, temporary
	...
	$(this.main).append(this.shortcutbar);
...
Nano.prototype.listShortcuts = function(tot){
	...
	shortStr += "</tr></table>";
	$(this.shortcutbar).html(shortStr);
}
...
Nano.prototype.find = function(srch){
	...
	if(ind > -1){
		...
		$(this.statusbar).html("<span>[ \""+srch+"\" not found ]</span>")
	}
}
